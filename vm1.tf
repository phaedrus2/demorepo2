resource "azurerm_virtual_machine" "web1" {
 ...
 os_profile {
        computer_name  = "user12web1"
        admin_username = "user12"    ## 아래 34번 라인의 계정이름과 동일해야 함
        admin_password = "qlalfqjsgh1!"     ## 12자리이상, 특수문자, 숫자, 대문자 조합으로 생성 필요
 ...
 os_profile_linux_config {
	    ...
	    ## ssh-keygen 명령어를 통해 Private Key(id_rsa)와 Public Key(id_rsa.pub)파일 생성
            ...
            path     = "/home/user12/.ssh/authorized_keys"   ## 가상 서버에 복사되는 위치
